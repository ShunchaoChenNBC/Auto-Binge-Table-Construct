...
and DATE(timestamp(post_cust_hit_time_gmt), "America/New_York") <= DATE_ADD(CURRENT_DATE(), INTERVAL -1 day))
SELECT 
Adobe_Tracking_ID,
Adobe_Date,
Adobe_Timestamp,
...
TIMESTAMP_ADD(adobe_timestamp , INTERVAL -40 second) as Adobe_Timestamp,--reduce the discrepancy between two tables
'' Player_Event,
'' Binge_Details,
...
where DATE(timestamp(adobe_date), "America/New_York") <= DATE_ADD(CURRENT_DATE(), INTERVAL -1 day)
order by 1, 2, 3;
